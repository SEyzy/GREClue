...
class PlotUtils:
            ...
            if training_stage != "val_thesis" or (training_stage == "val_thesis" and not hasattr(self, 'val_embb')):
                ...
                if UMAP:
                    umap_obj = umap.UMAP(
                        ...
                        n_components=2,
                        random_state=42,
                    ).fit(vae_means.detach().cuda())
                    E = umap_obj.embedding_
                    if centers is not None:
                        centers = umap_obj.transform(centers.cuda())
                else:
                    E = TSNE(n_components=2).fit_transform(vae_means.detach().cuda())
        else:
            E = vae_means.detach().cuda()
    ...
    def plot_decision_regions(
        ...
        yhat_maxed = yhat.max(axis=1).values.cuda()
