...
def init_mus_and_covs(codes, K, how_to_init_mu, logits, use_priors=True, prior=None, random_state=0, device="cpu"):
    ...
    if how_to_init_mu == "kmeans":
        if K == 1:
            kmeans = KMeans(n_clusters=K, random_state=random_state).fit(codes.detach().cuda())
        ...
        data_covs = compute_data_covs_hard_assignment(labels, codes, K, kmeans_mus.cuda(), prior)

        if use_priors:
            mus = prior.compute_post_mus(counts, kmeans_mus.cuda())
            covs = []
            for k in range(K):
        ...
        pca_codes = pca.fit_transform(codes.detach().cuda())
        ...
        kmeans_mus = torch.tensor(
            pca.inverse_transform(cluster_centers.cuda().numpy()),
            device=device,
            requires_grad=False,
        )
        ...
        data_covs = compute_data_covs_soft_assignment(logits, codes, K, mus.cuda(), prior.name)
...
def init_mus_and_covs_sub(codes, k, n_sub, how_to_init_mu_sub, logits, logits_sub, prior=None, use_priors=True, random_state=0, device="cpu"):
    if how_to_init_mu_sub == "kmeans":
        ...
        if use_priors:
            mus_sub = prior.compute_post_mus(counts, mus_sub.cuda())
            covs_sub = []
        ...
        if len(codes_k) <= n_sub:
            # empty cluster
            codes_k = codes
        pca = PCA(n_components=1).fit(codes_k.detach().cuda())
        pca_codes = pca.fit_transform(codes_k.detach().cuda())
...
def compute_data_covs_hard_assignment(labels, codes, K, mus, prior):
    if prior and prior.mus_covs_prior.name == "NIG":
        ...
        for k in range(K):
            ...
            if N_k > 0:
                cov_k = torch.matmul(
                    (codes_k - mus[k].cuda().repeat(len(codes_k), 1)).T,
                    (codes_k - mus[k].cuda().repeat(len(codes_k), 1)),
                )
                cov_k = cov_k / N_k
            else:
                if prior:
        ...
        for k in range(K):
            ...
            if indices.sum() < 2 or denominator[0] == 0 or denominator[1] == 0 or len(torch.unique(r_sub.argmax(-1))) < n_sub:
                ...
                for k_sub in range(n_sub):
                    z_sub = r_sub[:, k_sub]
                    mus_sub_k.append(
                        (z_sub.reshape(-1, 1) * codes_k.cuda()).sum(axis=0)
                        / denominator[k_sub]
                    )
    ...
    if hard_assignment:
        ...
        for k in range(K):
            ...
            if indices.sum() < 5:
                ...
                mus_sub_new.append(
                    mus_sub[2 * k: 2 * k + 2].clone().detach().cuda().type(torch.float32)
                )
            else:
                codes_k = codes[indices]
                for k_sub in range(n_sub):
                    if denominator[k_sub] == 0:
                        # empty cluster - do not change mu sub
                        mus_sub_new.append(
                            mus_sub[2 * k + k_sub].clone().detach().cuda().type(torch.float32).unsqueeze(0)
                        )
                    else:
...
def _create_subclusters(k_sub, codes, logits, logits_sub, mus_sub, pi_sub, n_sub, how_to_init_mu_sub, prior, device=None, random_state=0, use_priors=True):
    ...
    if how_to_init_mu_sub == "soft_assign":
        ...
        if how_to_init_mu_sub == "kmeans":
            ...
            # kmeans_1d
            pca = PCA(n_components=1).fit(codes_sub.detach().cuda())
            pca_codes = pca.fit_transform(codes_sub.detach().cuda())
